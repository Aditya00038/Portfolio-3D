...
export default function MinimalHero({
  brand = "NOVA",
  brandUrl = "https://codepen.io/RAFA3L",
  contactText = "Contact",
  ...
  heading = "Composable. Accessible. Production-ready.",
  desc = "Start with sensible defaults, ship without the noise, and scale your UI with confidence. Works across any app architecture.",
  showHeader = true,
  showHero = true,
  showContent = true,
  lineOpacity = 0.25
...
@keyframes drawX {
  ...
  60% { opacity: calc(var(--line-opacity) * 1.5); }
  100% { transform: scaleX(1); opacity: var(--line-opacity); }
}
@keyframes drawY {
  0% { transform: scaleY(0); opacity: 0; }
  60% { opacity: calc(var(--line-opacity) * 1.5); }
  100% { transform: scaleY(1); opacity: var(--line-opacity); }
}
@keyframes shimmer {
  0% { opacity: .0; }
  30% { opacity: .25; }
  ...
  max-width: 680px;
      ...
      {showHeader && (
        <header className="header">
          {brandUrl.startsWith("/") ? (
            <Link className="brand" to={brandUrl}>
              {brand}
            </Link>
          ) : (
            <a 
          ...
          <button className="cta" type="button" onClick={onContactClick}>
            {contactText}
          </button>
        </header>
      )}
