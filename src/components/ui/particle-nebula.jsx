...
export default function ParticleNebula({
  ...
  useEffect(() => {
    ...
    // Set pixel density ratio for razor-sharp rendering on High-DPI screens
    ...
    // Box-Muller transform to generate normally (Gaussian) distributed numbers
    const randomNormal = () => {
      let u = 0, v = 0;
    ...
    const makeParticle = () => {
      ...
      // Gaussian distributed radius ensures dense core and diffused tail
      const d = Math.abs(randomNormal()) * spread; 
      
      const baseX = cx + d * Math.cos(angle);
      const baseY = cy + d * Math.sin(angle);
      
      // Calculate opacity fade based on distance from core center
      ...
      return {
        ...
        angle,
        distance: d,
        size: Math.random() * 0.9 + 0.3, // Micro dots (granular texture)
        baseOpacity,
        opacity: baseOpacity,
    ...
    const animate = () => {
      ...
      particles.forEach((p) => {
        // Base organic motion: turbulent orbital and brownian sway
        ...
        // Interactive mouse physics: repulsion when cursor gets near
        if (mouse.active) {
        ...
        // Dynamic interpolation for smooth transition physics
        p.x += (targetX - p.x) * 0.12;
        p.y += (targetY - p.y) * 0.12;

        // Ambient stardust glimmer oscillation
    ...
    const handleMouseMove = (e) => {
      ...
      // Translate raw page coordinates to canvas viewport bounds
